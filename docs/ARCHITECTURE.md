...
Worker (Hono router)
   ...
   /api/machine/*            the same artifact routes for machines: bearer token ONLY, no
                              Access session accepted (requireApiToken). Meant to sit on an
                              Access Bypass policy so a scoped rtfx_… token is sufficient on
                              its own. Users/tokens/accounts are not mounted here.
...
- **API tokens** (`src/tokens.ts`) are the second authentication path, for callers that can't do
  ...
  allow-list. On `/api` this is *additional* to Cloudflare Access, which still gates the edge.
  On `/api/machine/*` it is the only gate, and correspondingly stricter: `requireApiToken`
  demands a bearer token and refuses a browser session outright, so an un-gated machine surface
  cannot be driven by a victim's cookies from another site. See `docs/HERMES_CLOUD.md` §2 and
  `docs/DEPLOY_RTFX.md` §5e.
