...
describe("through the mounted app, the 404 reveals nothing", () => {
  /**
   * The agent proved this at module level. This proves it through the real
   * route table, which is where a mounting mistake would show up: a real slug
   * and an invented one must produce byte-identical pages and byte-identical
   * POST responses.
   */
  it("serves an identical not-found page for a real and an invented slug", async () => {
    const body = new FormData();
    body.set("slug", "genuinely-here");
    body.set("title", "Here");
    body.set("visibility", "restricted");
    body.set("file", new File(["<p>x</p>"], "index.html", { type: "text/html" }));
    await req("/api/artifacts", { method: "POST", body, ...as("admin@test.com") });

    const anon = { headers: { "X-Dev-Anonymous": "true" } };
    const real = await (await req("/genuinely-here/", anon)).text();
    const fake = await (await req("/not-a-real-slug/", anon)).text();

    expect(real.replace(/genuinely-here/g, "SLUG")).toBe(fake.replace(/not-a-real-slug/g, "SLUG"));
  });

  it("answers a request identically for both", async () => {
    const post = (slug: string) =>
      req(`/_access-request/${slug}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Dev-Anonymous": "true" },
        body: JSON.stringify({ email: "curious@example.com" }),
      });
    const a = await post("genuinely-here");
    const b = await post("not-a-real-slug");
