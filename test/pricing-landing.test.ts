...
describe("pricing section", () => {
  ...
  it("points every tier's CTA at the same invite request the rest of the page uses, never a fake checkout", async () => {
    const html = await landing();
    for (const tier of ["free", "pro", "team"]) {
      const marker = `data-cta="pricing-${tier}"`;
      const idx = html.indexOf(marker);
      expect(idx, `${marker} missing`).toBeGreaterThan(-1);
      const tag = html.slice(html.lastIndexOf("<a", idx), idx + marker.length + 40);
      expect(tag).toContain('href="#waitlist"');
    }
  });
